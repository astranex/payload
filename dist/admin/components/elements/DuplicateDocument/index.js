...
const Duplicate = ({ slug, collection, id }) => {
        ...
        async (override = false) => {
            ...
            const create = async (locale) => {
                ...
                const response = await requests.get(
                    `${serverURL}${api}/${slug}/${id}?${localeParam}`
                );
                    ...
                    .forEach(async (locale) => {
                        if (!abort) {
                            const res = await requests.get(
                                `${serverURL}${api}/${slug}/${id}?locale=${locale}`
                            );
            ...
            toast.success(`${collection.labels.singular} успешно дублирован.`, {
                autoClose: 3000
            });
            setModified(false);
            setTimeout(() => {
    ...
    return React.createElement(
        ...
        modified &&
            ...
            React.createElement(
                Modal,
                { slug: modalSlug, className: `${baseClass}__modal` },
                React.createElement(
                    MinimalTemplate,
                    ...
                    React.createElement(
                        'p',
                        null,
                        'У вас есть несохранённые изменения. Вы точно хотите дублировать?'
                    ),
                    React.createElement(
                        Button,
                        {
                    ...
                    React.createElement(
                        Button,
                        { onClick: confirm, id: 'confirm-duplicate' },
                        'Дублировать без сохранения изменений'
                    )
                )
            )
    );
