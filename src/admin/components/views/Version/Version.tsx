...
const VersionView: React.FC<Props> = ({ collection, global }) => {
    ...
    const [localeOptions] = useState<LocaleOption[]>(() =>
        localization
    ...
    const [{ data: doc, isLoading }] = usePayloadAPI(versionFetchURL, {
    ...
    useEffect(() => {
        ...
        if (collection) {
            ...
            if (mostRecentDoc) {
                ...
                if (useAsTitle !== 'id') {
                    const titleField = collection.fields.find(
                        (field) =>
                            fieldAffectsData(field) && field.name === useAsTitle
            ...
            nav = [
                ...
                {
                    label: doc?.createdAt
                        ? format(new Date(doc.createdAt), dateFormat)
                        : ''
                }
            ];
        }
        ...
        <div className={baseClass}>
            ...
            <Gutter className={`${baseClass}__wrap`}>
                <div className={`${baseClass}__intro`}>
                    {doc?.autosave ? 'Autosaved version ' : 'Version'} created
                    on:
                </div>
