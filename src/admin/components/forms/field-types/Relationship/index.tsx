...
const Relationship: React.FC<Props> = (props) => {
            ...
            if (!errorLoading) {
                relationsToFetch.reduce(async (priorRelation, relation) => {
                    ...
                    if (resultsFetched < 10) {
                        ...
                        if (response.ok) {
                            const data: PaginatedDocs<unknown> =
                                await response.json();
                            ...
                            dispatchOptions({
                                type: 'ADD',
                                data: { docs: [] } as PaginatedDocs<unknown>,
                                relation,
                                hasMultipleRelations,
                                collection,
                                sort,
    ...
    const updateSearch = useDebouncedCallback(
        (searchArg: string, valueArg: unknown) => {
        ...
        >
            ...
            {!errorLoading && (
                <ReactSelect
                    ...
                    filterOption={
                        enableWordBoundarySearch
                            ? (item, searchFilter) => {
                                  const r = wordBoundariesRegex(
                                      searchFilter || ''
                                  );
