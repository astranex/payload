...
import buildObjectType, {
    ObjectTypeConfig
} from '../../graphql/schema/buildObjectType';
        ...
        if (collection.config.versions) {
            ...
            payload.Query.fields[`versions${pluralLabel}`] = {
                type: buildPaginatedListType(
                    `versions${formatName(pluralLabel)}`,
            ...
            payload.Mutation.fields[
                `restoreVersion${formatName(singularLabel)}`
            ] = {
                type: collection.graphQL.type,
                args: {
                    id: { type: GraphQLString }
        ...
        if (collection.config.auth) {
            collection.graphQL.JWT = buildObjectType({
                ...
                fields: collection.config.fields
                    .filter(
                        (field) => fieldAffectsData(field) && field.saveToJWT
                    )
                    .concat([
                        {
                            name: 'email',
